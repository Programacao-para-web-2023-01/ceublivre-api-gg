...
def updated_product(product: Product):
    wish_product = dbWishlist.fetch().items
    for item in wish_product:
        if item["price"] > product.price:
            wishlist_user = dbUsers.get(item["cod_user"])
            if wishlist_user:
                user_to_notify.append(wishlist_user)

    return {f"detail: os seguintes usuários receberão a notificação: users_to_notify"}
...
async def insert_product(product: Product):
    ...
    updated_product(new_item) #chamada da função criada para verificar preços da lista
...
async def update_product(update_product: Product):
    ...
    result = dbProducts.update(new_item, key=update_product.key)
...
async def insert_product_wishlist(iduser: str, idproduct: str):
    ...
    new_item = {
        ...
        "actual_price": dbProducts.get(idproduct)["price"] #recuperando o preço atual do produto
    }
    dbWishlist.insert(new_item)  #insert do novo item
